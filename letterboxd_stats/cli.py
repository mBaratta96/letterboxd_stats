...
def user_view_lb_export_lists_directory(lists_dir_path: str, limit: int = None, sort_ascending: bool = False) -> str:
    ...
    return user_view_lb_export_csv("Lists", os.path.join(lists_dir_path, list_names[name]), limit, sort_ascending, header=3)

def user_view_lb_export_csv(filetype: str, path: str, limit: int = None, sort_ascending: bool = False, header=0) -> str:
    ...
    df = USER_OPERATIONS_BY_EXPORT_TYPE[filetype](df, sort_ascending)
...
def _user_process_diary_df(dairy_df: pd.DataFrame, sort_ascending: bool = False) -> pd.DataFrame:
    dairy_df["Watched Date"] = pd.to_datetime(dairy_df["Watched Date"])
    sort_column = user_choose_option(dairy_df.columns.values.tolist(), "Select the order of your diary entries:")
    dairy_df.sort_values(by=sort_column, sort_ascending=sort_ascending, inplace=True)
    dairy_df = dairy_df.drop(["Rewatch", "Tags"], axis=1)
    return dairy_df

def _user_process_watchlist_df(df: pd.DataFrame, sort_ascending: bool = False) -> pd.DataFrame:
    ...
    if sort_column == "Shuffle":
        ...
        df.sort_values(by=sort_column, sort_ascending=sort_ascending, inplace=True)
    return df

def _user_process_ratings_df(df: pd.DataFrame, sort_ascending: bool = False) -> pd.DataFrame:
...
def _user_process_lists_df(df: pd.DataFrame, sort_ascending: bool = False) -> pd.DataFrame:
