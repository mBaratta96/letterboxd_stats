...
This module provides functionality for managing, processing, and visualizing Letterboxd export data in a CLI environment.
The `ExportViewer` class handles operations such as loading export files (e.g., diary, watchlist, ratings), processing them,
and rendering the results in a user-friendly format using the `CLIRenderer`.
...
Functions:
----------
- `get_list_name(list_csv_path: str) -> str`: Retrieves the name of a list from a Letterboxd export CSV file.
...
class ExportViewer:
    ...
    def __init__(self, root_folder: str, renderer:CLIRenderer, get_list_runtimes: bool = False, tmdb_api:TMDbAPI=None, lb_connector:LBPublicConnector=None):
        """
    ...
    def _process_diary_df(diary_df: pd.DataFrame, sort_ascending: bool = False) -> pd.DataFrame:
        ...
        logging.info(f"Diary DataFrame processed. Sorted by {sort_column}, ascending={sort_ascending}.")
        return diary_df
    ...
    def _process_watchlist_df(df: pd.DataFrame, sort_ascending: bool = False) -> pd.DataFrame:
        ...
        if sort_column == "Shuffle":
            ...
            logging.info(f"Watchlist DataFrame sorted by {sort_column}, ascending={sort_ascending}.")
        return df
    ...
    def _process_ratings_df(df: pd.DataFrame, sort_ascending: bool = False) -> pd.DataFrame:
        ...
        df.sort_values(by=sort_column, ascending=sort_ascending, inplace=True)
        logging.info(f"Ratings DataFrame sorted by {sort_column}, ascending={sort_ascending}.")
        if sort_column == "Rating":
            options = df["Rating"].unique().tolist()
            rating_filter  = UserInputHandler.user_choose_options_multiple(options=options)
            df = df[df["Rating"].isin(rating_filter )]
            logging.info(f"Filtered Ratings DataFrame to include ratings: {rating_filter}.")
        return df

    def _process_lists_df(self, list_df: pd.DataFrame, sort_ascending: bool = False) -> pd.DataFrame:
        ...
        sort_column = UserInputHandler.user_choose_option(list_df.columns.values.tolist(), "Select the order of your diary entries:")
        list_df.sort_values(by=sort_column, ascending=sort_ascending, inplace=True)

        logging.info(f"Lists DataFrame processed. Sorted by {sort_column}, ascending={sort_ascending}.")

        averages = {"Rating Mean": "{:.2f}".format(list_df["Rating"].mean())}

        if self.get_list_runtimes is True:
            self.renderer.render_string(f"Fetching movie runtimes...")
            tmdb_ids = list_df["Url"].parallel_map(self.lb_connector.get_tmdb_id_from_lb_page)
            list_df["Duration"] = tmdb_ids.parallel_map(self.tmdb_api.fetch_movie_runtime)  # type: ignore
            averages["Time-weighted Rating Mean"] = "{:.2f}".format(
                ((list_df["Duration"] / list_df["Duration"].sum()) * list_df["Rating"]).sum()
            )
            logging.info("Fetched runtimes and calculated time-weighted rating mean.")

        self.renderer.render_dict(averages, expand=False)
        logging.info(f"Averages calculated: {averages}.")
        return list_df
    ...
    def view_lb_export_csv(self, export_type: str, path: str, limit: int = None, header=0) -> str:
        ...
        try:
            ...
            selected_film = UserInputHandler.user_choose_film_from_list(df["Title"], df["Url"])
            ...
            logger.exception("An error occurred while processing the export")
            raise
    ...
    def add_lb_watched_status_column(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.exception("Failed to add watched status column")
            raise
